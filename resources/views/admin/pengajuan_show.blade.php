    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 30px;">
                            ...
                            <div style="display: flex; gap: 10px;">
                                ...
                                <form action="{{ route('admin.pengajuan.updateStatus', $pengajuan->id) }}" method="POST"
                                    ...
                                    <button type="submit" class="btn btn-success"
                                        onclick="return confirm('Anda yakin ingin MENERIMA pengajuan ini?')">
        ...
        <div>
            ...
            @if($pengajuan->perhitunganSmart)
                ...
                {{-- ================================================================ --}}
                {{-- === SALIN DAN GANTI KODE DARI SINI ... === --}}
                {{-- ================================================================ --}}

                @php
                    // Ambil KATEGORI yang sudah dihitung oleh Controller
                    $kategori = $hasil->kategori ?? 'Tidak Layak';
                    $style = '';

                    // Tentukan warna berdasarkan KATEGORI dari database, bukan skor
                    if ($kategori == 'Layak') {
                        // Style Sukses (Hijau)
                        $style = 'background-color: #d4edda; color: #155724;';

                    } elseif ($kategori == 'Kurang Layak') {
                        // Style Warning (Kuning/Oranye)
                ...
                <div style="padding: 15px; border-radius: 5px; margin-top: 20px; {{ $style }}">
                    ...
                    {{--
                    Tampilkan REKOMENDASI dan ALASAN langsung dari database
                    ($hasil->rekomendasi dan $hasil->alasan)
                    --}}
                    <strong style="font-size: 1.1rem;">{{ $hasil->rekomendasi }}</strong>
                    <p style="margin-top: 8px; margin-bottom: 0;">
                        <strong>Kategori:</strong> {{ $hasil->kategori }} <br>
                        <strong>Alasan:</strong> {{ $hasil->alasan }}
                    </p>
                </div>
            ...
            @else
                <div class="alert alert-danger"
                    style="background-color: #f8d7da; color: #721c24; padding: 15px; border-radius: 5px;">
